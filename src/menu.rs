...
/// initialized from the user's remembered preferences.
pub fn choose_clean(settings: &Settings) -> Option<CleanSelection> {
    let raw = RawMode::enter()?;
    let screen = AltScreen::enter();
    let style = Style::auto();
    let result = choose_clean_inner(&style, settings);
    drop(screen);
    drop(raw);
    result
}

fn choose_clean_inner(style: &Style, settings: &Settings) -> Option<CleanSelection> {
    let mut selected = [false; 6];
    for (i, (_, category)) in CLEAN_ITEMS.iter().enumerate() {
        selected[i] = category.is_some_and(|category| settings.categories.contains(&category));
    }
...
fn draw_clean(cursor: usize, selected: &[bool; 6], style: &Style, message: &str) {
    ...
    out.push_str(&format!(
        "\n  {}\n",
        style.dim(
            "↑↓/j/k move  •  Space toggle  •  Enter scan  •  q cancel  •  selection is remembered"
        )
    ));
    if !message.is_empty() {
