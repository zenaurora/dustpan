...
Windows 精简版 [Mole](https://github.com/tw93/Mole) —— `clean`（缓存清理）+ `analyze`（磁盘占用浏览）的 CLI。
...
对应 mole 的 `mo analyze`（analyze-go）：启动时并发扫描整棵目录树建立大小索引，之后逐层浏览零延迟。全屏 vim 式交互，单键即时响应（raw 模式，无需回车）：
...
刻意不清（重新下载代价高，与 mole 默认白名单一致）：`.m2/repository`、`.gradle/caches`、`.nuget/packages`、pnpm store。
...
- [ ] 交互式多选（对应 mole 的分页菜单）
- [ ] analyze 从浏览器内直接删除（需接入安全闸门 + 回收站）
- [ ] 回收站删除模式（Shell API 替代 PowerShell）
- [ ] 浏览器运行中检测
